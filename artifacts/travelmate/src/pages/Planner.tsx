...
function transportBadgeClass(mode: string) {
  switch (mode) {
    case "walking":          return "text-emerald-400 bg-emerald-500/10 border-emerald-500/25";
    case "car":              return "text-orange-400 bg-orange-500/10 border-orange-500/25";
    case "public_transport": return "text-sky-400 bg-sky-500/10 border-sky-500/25";
...
function transportIcon(mode: string, size = "w-3 h-3") {
  switch (mode) {
    ...
    case "car":              return <Car className={size} />;
    case "public_transport": return <Bus className={size} />;
    default:                 return null;
  }
}
...
export function Planner() {
  ...
  const handleRegenerateDay = async (dayNumber: number) => {
    ...
    try {
      const response = await generateMutation.mutateAsync({
        ...
        // Pass existing days so backend can exclude their POIs
      });
      // Merge just the regenerated day back in
      const regeneratedDay = response.days.find((d) => d.dayNumber === dayNumber);
      if (regeneratedDay) {
        const merged: ItineraryResponse = {
          ...result,
          days: result.days.map((d) => (d.dayNumber === dayNumber ? regeneratedDay : d)),
  ...
  if (result) {
    ...
    // Type-extended fields from backend (not in generated types yet)
    const dayFreePois = (dayData as any).freePoisCount ?? dayData.pois.filter((p) => p.isFree).length;
    const dayPaidPois = (dayData as any).paidPoisCount ?? dayData.pois.filter((p) => !p.isFree).length;
          ...
          <div className="p-5 border-b border-border/60 bg-card/60 shrink-0">
            ...
            <div className="flex items-start justify-between gap-3 mb-3">
              <div className="min-w-0">
                <h2 className="text-2xl font-display font-bold leading-tight truncate">
                  ...
                  {result.country && result.country !== result.city && (
                    <span className="text-muted-foreground font-normal text-base">, {result.country}</span>
                  )}
                </h2>
                ...
                <div className="flex flex-wrap items-center gap-1.5 mt-2">
                  ...
                  <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border ${transportBadgeClass(result.transportMode)}`}>
                    {transportIcon(result.transportMode)}
                    {transportLabel(result.transportMode)}
                  </span>
                  <span className="inline-flex items-center gap-1 text-xs text-muted-foreground bg-white/5 border border-white/8 px-2 py-0.5 rounded-full">
                    {profileEmoji(request.travelProfile ?? "couple")} {(request.travelProfile ?? "couple")}
                  </span>
                  <span className="inline-flex items-center gap-1 text-xs text-muted-foreground bg-white/5 border border-white/8 px-2 py-0.5 rounded-full">
                    {rhythmEmoji(request.travelRhythm ?? "balanced")} {(request.travelRhythm ?? "balanced")}
                  </span>
                </div>
              </div>
              ...
              <div className="text-right shrink-0">
                ...
                <p className="text-2xl font-bold text-accent">{formatCurrency(result.totalEstimatedCost)}</p>
                {budgetDefined && (
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="rounded-2xl border border-white/8 bg-white/3 p-4 mb-1">
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-3">
                    ...
                    <span className="text-xs text-muted-foreground flex items-center gap-1">
                      ...
                      {Math.round((dayData.totalDuration ?? 0) / 60)}h visit
                      {(dayData as any).travelDuration > 0 && (
                  ...
                  {/* Transport summary */}
                  <div className={`mt-2.5 inline-flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg border ${transportBadgeClass(result.transportMode)}`}>
                    {transportIcon(result.transportMode)}
                    {(dayData as any).transportSummary ?? ""}
                  </div>
                </div>

                {/* POI cards + segment connectors */}
                    ...
                    {idx < dayData.pois.length - 1 && (dayData as any).segments?.[idx] && (
                      <div className="flex items-center gap-2 py-0.5 pl-4 pr-2">
                        <div className="flex flex-col items-center gap-0.5">
                          ...
                          <div className={`p-1 rounded-full border ${transportBadgeClass(result.transportMode)}`}>
                ...
                <div className="pt-4 pb-6">
                  <Button variant="outline" className="w-full text-muted-foreground" onClick={reset}>
            ...
            >
              ...
              {step === 1 && (
                <div className="space-y-8">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-widest text-primary mb-2">Step 1 of 4</p>
                    <h2 className="text-3xl md:text-4xl font-display font-bold mb-2">Where to?</h2>
                    <p className="text-muted-foreground">Enter your destination and we'll find the best it has to offer.</p>
                  </div>
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="text-sm font-medium text-muted-foreground mb-2 block">
                        Country <span className="text-muted-foreground/40 font-normal">(optional — helps with smaller cities)</span>
                      </label>
                      <Input
              ...
              {step === 2 && (
                <div className="space-y-8">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-widest text-primary mb-2">Step 2 of 4</p>
                    <h2 className="text-3xl font-display font-bold mb-2">Time & Budget</h2>
                    <p className="text-muted-foreground">How long are you visiting, and what's your total spend?</p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      {BUDGET_PRESETS.map((preset) => (
                        <button
                          key={preset}
                          onClick={() => { setBudgetInput(String(preset)); updateRequest({ budgetAmount: preset }); }}
                      ...
                      <button
                        onClick={() => { setBudgetInput("0"); updateRequest({ budgetAmount: 0 }); }}
                        className={`px-3 py-1.5 rounded-lg text-sm font-semibold border-2 transition-all ${
                    ...
                    <div className="relative">
                      <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground font-semibold text-lg pointer-events-none">€</span>
                      <Input
                        type="number" min={0}
                    ...
                    {(request.budgetAmount ?? 0) > 0 && (
                      <div className="mt-2 p-3 rounded-xl bg-white/3 border border-white/8 text-xs text-muted-foreground">
                        <span className="font-semibold text-foreground">
                          {formatCurrency(Math.round((request.budgetAmount ?? 0) / (request.days ?? 1)))} / day
                        </span>
                        {(request.budgetAmount ?? 0) < 60 && " — free attractions prioritised, minimal paid entry"}
                        {(request.budgetAmount ?? 0) >= 60 && (request.budgetAmount ?? 0) < 160 && " — mix of free and affordable paid attractions"}
                        {(request.budgetAmount ?? 0) >= 160 && " — full access to premium and paid experiences"}
                      </div>
                    )}
                    {(request.budgetAmount ?? 0) === 0 && (
                      <p className="mt-2 text-xs text-muted-foreground/60">No limit — best attractions selected regardless of entry price.</p>
                    )}
                  </div>
                </div>
              )}

              {/* ── STEP 3: Profile + Rhythm ── */}
              {step === 3 && (
                <div className="space-y-8">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-widest text-primary mb-2">Step 3 of 4</p>
                  ...
                  <div>
                    <label className="text-sm font-medium text-muted-foreground mb-3 block">Travelling as</label>
                    <div className="grid grid-cols-2 gap-3">
                      {PROFILES.map((p) => (
                        ...
                        >
                          ...
                          <div className={`font-bold text-sm ${request.travelProfile === p.id ? "text-primary" : "text-foreground"}`}>{p.label}</div>
                      ...
                      {RHYTHMS.map((r) => (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <div className={`font-bold text-sm ${request.travelRhythm === r.id ? "text-primary" : "text-foreground"}`}>{r.label}</div>
                            <div className="text-xs text-muted-foreground mt-0.5">{r.desc}</div>
                          </div>
                          {request.travelRhythm === r.id && <Sparkles className="w-4 h-4 text-accent shrink-0" />}
                        </button>
                      ))}
                    </div>
                  </div>
              ...
              {/* ── STEP 4: Transport + Interests + Priority + Start time ── */}
              {step === 4 && (
                <div className="space-y-8">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-widest text-primary mb-2">Step 4 of 4</p>
                    <h2 className="text-3xl font-display font-bold mb-2">Final Touches</h2>
                    <p className="text-muted-foreground">Set your transport, preferred start time, attraction style, and interests.</p>
                  </div>

                  {/* Transport mode */}
                  <div>
                    <label className="text-sm font-medium text-muted-foreground mb-3 block">Transport Mode</label>
                    <div className="space-y-2.5">
                      {TRANSPORTS.map((t) => {
                        const Icon = t.icon;
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className={`font-bold text-sm ${request.transportMode === t.id ? "text-primary" : "text-foreground"}`}>{t.label}</div>
                              <div className="text-xs text-muted-foreground mt-0.5">{t.desc}</div>
                            </div>
                            {request.transportMode === t.id && <Sparkles className="w-4 h-4 text-accent shrink-0" />}
                          </button>
                        );
                      })}
                    </div>
                  ...
                  {/* Start time */}
                  <div>
                    <label className="text-sm font-medium text-muted-foreground mb-3 block">Preferred Start Time</label>
                    <div className="grid grid-cols-3 gap-2.5">
                      {START_TIMES.map((s) => (
                        <button
                          key={s.id}
                          onClick={() => updateRequest({ startTime: s.id as ItineraryRequest["startTime"] })}
                        ...
                        >
                          ...
                          <div className={`font-bold text-xs ${request.startTime === s.id ? "text-primary" : "text-foreground"}`}>{s.label}</div>
                          <div className="text-xs text-muted-foreground/70 mt-0.5 leading-tight">{s.desc}</div>
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Priority mode */}
                  <div>
                    <label className="text-sm font-medium text-muted-foreground mb-3 block">Attraction Style</label>
                    <div className="grid grid-cols-2 gap-2.5">
                      {PRIORITY_MODES.map((m) => (
                        <button
                          key={m.id}
                          onClick={() => updateRequest({ priorityMode: m.id as ItineraryRequest["priorityMode"] })}
                          className={`p-4 rounded-2xl border-2 transition-all text-left ${
                        ...
                        >
                          ...
                          <div className={`font-bold text-sm leading-tight ${request.priorityMode === m.id ? "text-primary" : "text-foreground"}`}>{m.label}</div>
                          <div className="text-xs text-muted-foreground mt-1 leading-tight">{m.desc}</div>
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="text-sm font-medium text-muted-foreground mb-3 block">
                      Interests <span className="text-muted-foreground/40 font-normal">(optional — fine-tunes selection)</span>
                    </label>
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              ...
                              updateRequest({
                                interests: isSelected ? curr.filter((i) => i !== id) : [...curr, id as any],
                              });
                            }}
              ...
              <div className="mt-8 flex items-center justify-between pt-6 border-t border-white/8">
                ...
                {step < 4 ? (
                  ...
                  <Button size="lg" onClick={handleGenerate} className="bg-gradient-to-r from-primary to-accent border-none shadow-lg shadow-primary/25">
